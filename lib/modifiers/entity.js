...
function insertEntity(editorState, type, attrs, content) {
    ...
    return editorState.applyTransform(function(editor) {
        ...
        if (nodeType.isText) {
            ...
            newNode = nodeType.createAndFill(
                attrs,
                _createContent(editorState, content)
            );
        }
...
/**
 * Append an entity at the end of the document, select it and scroll to it.
 * @param {EditorState} editorState
 * @param {String} type
 * @param {Object} attrs
 * @param {String|Node|undefined} content
...
function appendEntityAndFocus(editorState, type, attrs, content) {
    ...
    return editorState.applyTransform(function(editor) {
        ...
        var end = editor.doc.nodeSize - 2; // at the end of the doc

        editor.tr.insert(end, nodeType.createAndFill(attrs, content)).apply();

        var contentSize = typeof content === 'string' ? content.length : content.nodeSize;

        // end + 1 to be inside the created node
        var selectStart = end + 1;
        var selectEnd = selectStart + contentSize;
        editor.setTextSelection(selectStart, selectEnd);
        editor.scrollIntoView(selectStart);
    });
}
